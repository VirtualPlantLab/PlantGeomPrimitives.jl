...
function Base.iterate(t::TV, i::Int = 1)::Union{Nothing,Tuple{eltype(TV),Int64}} where {TV<:TrapezoidVertices}
...
Base.length(r::TrapezoidVertices) = 6
function Base.eltype(::Type{TrapezoidVertices{TT,VT}}) where {TT,VT}
    @inbounds VT.types[1]
end
...
Trapezoid(trans::CT.AbstractAffineMap, ratio) =
