...
struct SolidCubeMeshVertices{VT,TT}
    trans::TT
    verts::VT
end
function SolidCubeMeshVertices(trans)
    FT = eltype(trans.linear)
    SolidCubeMeshVertices(trans, all_solid_cube_vertices(FT))
end
function Base.iterate(c::SCV,)::Union{Nothing,Tuple{eltype(SCV),Int64}} where {SCV<:SolidCubeMeshVertices}
    (@inbounds c.trans(c.verts[1]), 2)
end
function Base.iterate(c::SCV,i,)::Union{Nothing,Tuple{eltype(SCV),Int64}} where {SCV<:SolidCubeMeshVertices}
    i > 36 ? nothing : (@inbounds c.trans(c.verts[i]), i + 1)
end
Base.length(c::SolidCubeMeshVertices) = 36
function Base.eltype(::Type{SolidCubeMeshVertices{VT,TT}}) where {VT,TT}
    @inbounds VT.types[1]
end
...
"""
    SolidCubeMesh(;length = 1.0, width = 1.0, height = 1.0)
...
# Examples
```jldoctest
julia> SolidCubeMesh(;length = 1.0, width = 1.0, height = 1.0);
```
"""
function SolidCubeMesh(; length::FT = 1.0, width::FT = 1.0, height::FT = 1.0) where {FT}
    SolidCubeMesh(CT.LinearMap(CT.SDiagonal(height / FT(2), width / FT(2), length)))
end

# Create a solid_cube from affine transformation
SolidCubeMesh(trans::CT.AbstractAffineMap) = PrimitiveMesh(trans, SolidCubeMeshVertices)

# Create a solid_cube from affine transformation and add it in-place to existing mesh
SolidCube!(m::Mesh, trans::CT.AbstractAffineMap) = Primitive!(m, trans, SolidCubeMeshVertices)
