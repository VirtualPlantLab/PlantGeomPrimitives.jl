...
struct SolidCylinderMeshVertices{FT,TT}
    n::Int
    Δ::FT
    trans::TT
end
function SolidCylinderMeshVertices(n, trans)
    FT = eltype(trans.linear)
    SolidCylinderMeshVertices(n, FT(2pi / n), trans)
end
function Base.iterate(c::SolidCylinderMeshVertices{FT,TT},i::Int = 1)::Union{Nothing,Tuple{Vec{FT},Int64}} where {FT,TT}
...
Base.length(c::SolidCylinderMeshVertices) = 12c.n
Base.eltype(::Type{SolidCylinderMeshVertices{FT,TT}}) where {FT,TT} = Vec{FT}


"""
    SolidCylinderMesh(;length = 1.0, width = 1.0, height = 1.0, n = 80)
...
# Examples
```jldoctest
julia> SolidCylinderMesh(;length = 1.0, width = 1.0, height = 1.0, n = 80);
```
"""
function SolidCylinderMesh(;length::FT = 1.0, width::FT = 1.0, height::FT = 1.0, n::Int = 80) where {FT}
    trans = CT.LinearMap(CT.SDiagonal(height / FT(2), width / FT(2), length))
    SolidCylinderMesh(trans, n = n)
end

# Create a SolidCylinder from affine transformation
function SolidCylinderMesh(trans::CT.AbstractAffineMap; n::Int = 80)
    @assert iseven(n)
    n = div(n, 4)
    Primitive(trans, x -> SolidCylinderMeshVertices(n, x))
end
...
function SolidCylinder!(m::Mesh, trans::CT.AbstractAffineMap; n::Int = 80)
    @assert iseven(n)
    n = div(n, 4)
    Primitive!(m, trans, x -> SolidCylinderMeshVertices(n, x))
end
