...
struct HollowFrustumMeshVertices{FT,TT}
    ratio::FT
    n::Int
    Δ::FT
    trans::TT
end
function HollowFrustumMeshVertices(ratio::FT, n, trans) where {FT}
    @assert eltype(trans.linear) == FT
    HollowFrustumMeshVertices(ratio, n, FT(2 * pi / n), trans)
end
function Base.iterate(c::HollowFrustumMeshVertices{FT,TT}, i::Int = 1)::Union{Nothing,Tuple{Vec{FT},Int64}} where {FT,TT}
...
Base.length(c::HollowFrustumMeshVertices) = 6c.n
Base.eltype(::Type{HollowFrustumMeshVertices{FT,TT}}) where {FT,TT} = Vec{FT}


"""
    HollowFrustumMesh(;length = 1.0, width = 1.0, height = 1.0, ratio = 1.0, n = 40)
...
# Examples
```jldoctest
julia> HollowFrustumMesh(;length = 1.0, width = 1.0, height = 1.0, n = 40, ratio = 0.5);
```
"""
function HollowFrustumMesh(;length::FT = 1.0, width::FT = 1.0, height::FT = 1.0,ratio::FT = 1.0,
                       n::Int = 40) where {FT}
    trans = CT.LinearMap(CT.SDiagonal(height / FT(2), width / FT(2), length))
    HollowFrustumMesh(ratio, trans, n = n)
end

# Create a HollowFrustum from affine transformation
function HollowFrustumMesh(ratio, trans::CT.AbstractAffineMap; n::Int = 40)
    @assert iseven(n)
    n = div(n, 2)
    PrimitiveMesh(trans, x -> HollowFrustumMeshVertices(ratio, n, x))
end
...
function HollowFrustum!(m::Mesh, ratio, trans::CT.AbstractAffineMap; n::Int = 40)
    @assert iseven(n)
    n = div(n, 2)
    Primitive!(m, trans, x -> HollowFrustumMeshVertices(ratio, n, x))
end
