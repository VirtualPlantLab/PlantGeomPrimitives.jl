...
struct HollowCylinderMeshVertices{FT,TT}
    n::Int
    Δ::FT
    trans::TT
end
function HollowCylinderMeshVertices(n, trans)
    FT = eltype(trans.linear)
    HollowCylinderMeshVertices(n, FT(2pi / n), trans)
end
function Base.iterate(c::HollowCylinderMeshVertices{FT,TT}, i::Int = 1)::Union{Nothing,Tuple{Vec{FT},Int64}} where {FT,TT}
...
Base.length(c::HollowCylinderMeshVertices) = 6c.n
Base.eltype(::Type{HollowCylinderMeshVertices{FT,TT}}) where {FT,TT} = Vec{FT}


"""
    HollowCylinderMesh(;length = 1.0, width = 1.0, height = 1.0, n = 40)
...
# Examples
```jldoctest
julia> HollowCylinderMesh(;length = 1.0, width = 1.0, height = 1.0, n = 40);
```
"""
function HollowCylinderMesh(;
    length::FT = 1.0,
    width::FT = 1.0,
    height::FT = 1.0,
    n::Int = 40,
    ...
    HollowCylinderMesh(trans, n = n)
end

# Create a HollowCylinder from affine transformation
function HollowCylinderMesh(trans::CT.AbstractAffineMap; n::Int = 40)
    @assert iseven(n)
    n = div(n, 2)
    PrimitiveMesh(trans, x -> HollowCylinderMeshVertices(n, x))
end
...
function HollowCylinder!(m::Mesh, trans::CT.AbstractAffineMap; n::Int = 40)
    @assert iseven(n)
    n = div(n, 2)
    Primitive!(m, trans, x -> HollowCylinderMeshVertices(n, x))
end
