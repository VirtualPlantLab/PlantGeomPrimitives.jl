...
struct SolidFrustumMeshVertices{FT,TT}
    ratio::FT
    n::Int
    Δ::FT
    trans::TT
end
function SolidFrustumMeshVertices(ratio::FT, n, trans) where {FT}
    @assert eltype(trans.linear) == FT
    SolidFrustumMeshVertices(ratio, n, FT(2 * pi / n), trans)
end
function Base.iterate(c::SolidFrustumMeshVertices{FT,TT},i::Int = 1)::Union{Nothing,Tuple{Vec{FT},Int64}} where {FT,TT}
    # Odd triangles
    if i < 3*c.n + 1
        j = div(i - 1, 3) + 1 # 3:n
        v = mod(i - 1 , 3) + 1
...
Base.length(c::SolidFrustumMeshVertices) = 12c.n
Base.eltype(::Type{SolidFrustumMeshVertices{FT,TT}}) where {FT,TT} = Vec{FT}


"""
    SolidFrustumMesh(;length = 1.0, width = 1.0, height = 1.0, ratio = 1.0, n = 40)
...
# Examples
```jldoctest
julia> SolidFrustumMesh(;length = 1.0, width = 1.0, height = 1.0, n = 40, ratio = 0.5);
```
"""
function SolidFrustumMesh(;
    length::FT = 1.0,
    width::FT = 1.0,
    height::FT = 1.0,
    ratio::FT = 1.0,
    ...
    SolidFrustumMesh(ratio, trans, n = n)
end

# Create a SolidFrustum from affine transformation
function SolidFrustumMesh(ratio, trans::CT.AbstractAffineMap; n::Int = 40)
    @assert iseven(n)
    n = div(n, 4)
    PrimitiveMesh(trans, x -> SolidFrustumMeshVertices(ratio, n, x))
end
...
function SolidFrustum!(m::Mesh, ratio, trans::CT.AbstractAffineMap; n::Int = 40)
    @assert iseven(n)
    n = div(n, 4)
    Primitive!(m, trans, x -> SolidFrustumMeshVertices(ratio, n, x))
end
